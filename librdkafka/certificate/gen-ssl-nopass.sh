...
EOF
	...
	[[ -f ${PFX}client.truststore.jks ]] || keytool -keystore ${PFX}client.truststore.jks -alias CARoot -import -file $CA_CERT <<EOF
yes
EOF

	echo "############ Generating key"
	keytool -keystore ${PFX}client.keystore.jks -alias localhost -validity $VALIDITY -genkey -keyalg RSA <<EOF
$CN
$OU
$O
$L
...
EOF
	echo "########### Export certificate"
	keytool -keystore ${PFX}client.keystore.jks -alias localhost -certreq -file ${PFX}cert-file

	echo "########### Sign certificate"
	openssl x509 -req -CA ${CA_CERT} -CAkey ${CA_CERT}.key -in ${PFX}cert-file -out ${PFX}cert-signed -days $VALIDITY -CAcreateserial	

	echo "########### Import CA"
	keytool -keystore ${PFX}client.keystore.jks -alias CARoot -import -file ${CA_CERT} <<EOF
yes
EOF

	echo "########### Import signed CA"
	keytool -keystore ${PFX}client.keystore.jks -alias localhost -import -file ${PFX}cert-signed

    else
	# Standard OpenSSL keys
	echo "11111############ Generating key"
	openssl genrsa -out ${PFX}client.key 2048 
	
	echo "############ Generating request"
	openssl req -key ${PFX}client.key -new -out ${PFX}client.req \
		<<EOF
$C
$ST
$L
...
EOF
	...
	echo "########### Signing key"
	openssl x509 -req -in ${PFX}client.req -CA $CA_CERT -CAkey ${CA_CERT}.key -CAserial ${CA_CERT}.srl -out ${PFX}client.pem -days $VALIDITY
